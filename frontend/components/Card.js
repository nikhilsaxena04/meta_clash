...
export default function Card({ card, faceDown = false, selected = false, layoutId }) {
  if (!card && !faceDown) return <div className="w-[120px] h-[180px] md:w-48 md:h-72 lg:w-64 lg:h-96 bg-white/5 rounded-2xl border-2 border-dashed border-white/10 animate-pulse" />;
  ...
  return (
    <motion.div
      ...
      className={`relative w-[120px] h-[180px] md:w-48 md:h-72 lg:w-64 lg:h-96 rounded-2xl ${selected ? 'shadow-[0_0_40px_rgba(250,204,21,0.5)] scale-105 z-10' : 'shadow-xl'}`}
    >
      ...
      >
        {card && (
          <>
            ...
            {/* Stats Section */}
            <div className="flex-1 bg-black/60 p-2 md:p-4 flex flex-col justify-center border-t border-white/10 relative z-20">
              <div className="font-bold text-white text-base md:text-xl mb-1 md:mb-3 truncate drop-shadow-md">{card.name}</div>
